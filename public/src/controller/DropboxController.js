...
import * as element from './listElements.js';

export class DropboxController {
  constructor() {
    this.onSelectChange = new Event('selectChange');
    this.btnSendFileEl = document.querySelector(element.BTN_SEND_FILE);
    this.filesEl = document.querySelector(element.FILES);
    this.snack_bar_react = document.querySelector(element.MODAL_SNACK_BAR);
    this.snack_progresbar = this.snack_bar_react.querySelector(element.PROGRESSBAR_EL);
    this.name_file = this.snack_bar_react.querySelector(element.NAME_FILE);
    this.time_left = this.snack_bar_react.querySelector(element.TIME_PROGRESS);
    this.list_of_files_and_directories = document.querySelector(element.LIST_OF_FILES_DIRECTORIES);
    this.btn_delete = document.querySelector(element.BTN_DELETE);
    this.btn_rename = document.querySelector(element.BTN_RENAME);
    this.btn_new_folder = document.querySelector(element.BTN_NEW_FOLDER);
  ...
  connectFirebase() {
    const firebaseConfig = {
      apiKey: 'AIzaSyAI6TaATEGEWHxU-dKMzAiDoGPwJ4oi-9M',
      authDomain: 'dropbox-clone-137cc.firebaseapp.com',
      databaseURL: 'https://dropbox-clone-137cc-default-rtdb.firebaseio.com',
      projectId: 'dropbox-clone-137cc',
      storageBucket: 'dropbox-clone-137cc.appspot.com',
      messagingSenderId: '880941777539',
      appId: '1:880941777539:web:c73822c6b9808c319751b6',
      measurementId: 'G-RP39DM3ZYW'
    };
  ...
  getSelection() {
    return this.list_of_files_and_directories.querySelectorAll('.selected');
  }

  showButton(statusDelete, statusRaname) {
    this.btn_delete.style.visibility = statusDelete;
    this.btn_delete.style.height = statusDelete === 'visible' ? 'auto' : 0;
    this.btn_delete.style.opacity = statusDelete === 'visible' ? 1 : 0;

    this.btn_rename.style.visibility = statusRaname;
    this.btn_rename.style.height = statusRaname === 'visible' ? 'auto' : 0;
    this.btn_rename.style.opacity = statusRaname === 'visible' ? 1 : 0;
  }
  ...
  initEvents() {
    ...
    this.filesEl.addEventListener('change', async ({ target }) => {
      this.btnSendFileEl.disabled = true;
      try {
        const { files } = target;
        const responses = await this.uploadTask(files);

        responses.forEach(async (resp) => {
          const file = resp.files['input-file'];
    ...
    this.list_of_files_and_directories.addEventListener('selectChange', (e) => {
      const { length } = this.getSelection();
      switch (length) {
        case 0:
          this.showButton('hidden', 'hidden');
          break;
        case 1:
          this.showButton('visible', 'visible');
          break;
        default:
          this.showButton('visible', 'hidden');
      }
    });

    this.btn_rename.addEventListener('click', (event) => {
      const liSelected = this.list_of_files_and_directories.querySelector('.selected');
      const keySelected = liSelected.getAttribute('data-key');
      const file = this.getListAllFiles()[keySelected];

      const newName = window.prompt('Digite o nome do aquivo', file.originalFilename);
      if (newName) {
        file.originalFilename = newName;
        set(ref(this.database, `files/${keySelected}`), file);
      }
  ...
  readFilesInDB() {
    const starCountRef = ref(this.database, 'files');

    return onValue(starCountRef, (snapshot) => {
      this.list_of_files_and_directories.innerHTML = '';
      const snap = snapshot.val();
      Object.entries(snap).map((data) => {
        const [id, file] = data;
        this.list_of_files_and_directories.appendChild(this.getfileview(file, id));
      });
    });
  }
  ...
  async addFile(file) {
    ...
    await set(ref(this.database, `files/${_id}`), file);
  }
          ...
          try {
            ...
            const { data, ...rest } = await axios.request({
              ...
              onUploadProgress: (event) => {
                ...
                this.uploadProgress({ loaded, total, file });
              }
            });
            resolve(data);
          } catch (error) {
  ...
  uploadProgress({ loaded, total, file }) {
    ...
    const time_spent = Date.now() - this.start_upload;
    const time_left_await = ((100 - progress) * time_spent) / progress;

    this.snack_progresbar.style.width = `${progress}%`;
    this.name_file.innerHTML = file.name;
    this.time_left.innerHTML = this.formatTime(Number(time_left_await));
  }

  showModal(show = false) {
    this.snack_bar_react.style.display = show ? 'block' : 'none';
  }
  ...
  formatTime(duration) {
    ...
    return `${parseInt(duration).toFixed(3)} ms`;
  }

  getFileIConView({ mimetype, ...file }) {
    switch (mimetype) {
      case 'folder':
        return `
  ...
  getfileview(file, key) {
    ...
    li.innerHTML = `
      ...
      <div class="name text-center text-ellipsis">${file.originalFilename}</div>
    `;

    return li;
  }

  foundIndexs(array, element) {
    return array.findIndex((child) => child.getAttribute('data-key') === element.getAttribute('data-key'));
  }

  multipleSeleted({ li, firstLi }) {
    const {
      parentElement: { childNodes }
    } = li;
    const childs = [...childNodes];
    const [stardIndex, lastIndex] = [this.foundIndexs(childs, firstLi), this.foundIndexs(childs, li)].sort();

    childs.forEach((child, index) => index >= stardIndex && index <= lastIndex && child.classList.add('selected'));
    this.list_of_files_and_directories.dispatchEvent(this.onSelectChange);
  }
  ...
  eventsLi(li) {
    li.addEventListener('click', (event) => {
      // when user shift
      if (event.shiftKey) {
        const firstLi = this.list_of_files_and_directories.querySelector('.selected');
        if (firstLi) return this.multipleSeleted({ li, firstLi });
      }
      // single selected
      if (!event.ctrlKey) {
        this.list_of_files_and_directories.querySelectorAll('li.selected').forEach((list) => list.classList.remove('selected'));
